...
* **Security:** Leveraged Spring Boot `@Configuration` files to guard against XSS attacks based on the various input fields of the application. The image storage also has a basic-level security defined with the tandem of `SecurityConfig and WebConfig`. Field validators were also applied in both creation and search payloads.
...
* **UI State Synchronicity:** Actions like delete propagation and unsaved changes warning are not fully implemented in the React UI due to time constrains. V2 would also address this limitation.
